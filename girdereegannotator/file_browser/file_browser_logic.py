...
class FileBrowserLogic:
    ...
    def _load_eeg_files(self) -> None:
        if self.data.eeg_file is None:
            raise AnnotatorLoadingError("EEG file is missing")
    ...
    def _load_slicer_files(self) -> None:
        if len(self.data.slicer_files) == 0:
            raise AnnotatorLoadingError("Slicer file is missing")
        ...
        try:
            ...
            raise AnnotatorLoadingError("Could not load data into Slicer Annotator") from e
