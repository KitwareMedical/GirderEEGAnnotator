...
from trame_alerts.core.service import AlertType
...
class EEGViewerError(Exception):
    def __init__(self, alert_type: AlertType = "error", *args):
        super().__init__(*args)
    ...
    def _save_annotations_file(
        ...
        try:
            ...
            if eeg_fileset.is_validated:
                raise EEGViewerError(
                    f"{eeg_fileset.name} is validated and cannot accept modifications.", alert_type="warning"
                )
